...
import { getAllReservations, acceptEquipment, rejectEquipment } from '../../services/api' 
...
function StaffDashboard() {
  ...
  useEffect(() => {
    getAllReservationsHandler(debouncedSearchQuery, currentPage, selectedCategory, selectedStatus)
  }, [debouncedSearchQuery, currentPage, selectedCategory, selectedStatus])

  function getAllReservationsHandler(searchQuery, page = 1, category = '', status = '') {
    setLoading(true)
    const searchParam = searchQuery && searchQuery.trim() ? searchQuery.trim() : null
    getAllReservations(searchParam, page, 10, category || null, status || null).then(res => {
  ...
  // Get unique categories from reservations
  const categories = allReservations.length > 0 
    ? [...new Set(allReservations.map(res => res.equipment?.category).filter(Boolean))].sort()
    : []

  function acceptEquipmentHandler(reservationID) {
    acceptEquipment(reservationID).then(res => {
      if (res.status === "success") {
  ...
  function rejectEquipmentHandler(reservationID) {
    rejectEquipment(reservationID).then(res => {
      if (res.status === "success") {
        toast.success(res.message)
        getAllReservationsHandler(debouncedSearchQuery, currentPage, selectedCategory, selectedStatus)
      } else {
  ...
  return (
    <div className='py-4 px-4 sm:px-6 lg:px-10 max-w-7xl mx-auto'>
      <h1 className='text-2xl sm:text-3xl font-semibold mb-6'>Pending Equipment Requests</h1>
      ...
      <div className='mb-4 space-y-4'>
        ...
        <div className='relative'>
          ...
          <input 
            type="text" 
            placeholder="Search by equipment name, category, or student name..." 
            value={searchQuery} 
      ...
      {!loading && (
        <div className='mb-4 text-sm text-gray-600'>
          Showing {allReservations.length} of {pagination.totalCount} reservation{pagination.totalCount !== 1 ? 's' : ''} 
          ...
          <table className='w-full text-sm min-w-[800px]'>
            <thead className="bg-secondary border-b border-gray-200">
              <tr className="text-left">
                ...
                <th className="py-3 px-3 sm:px-4 font-medium text-black whitespace-nowrap">Start Date</th>
                <th className="py-3 px-3 sm:px-4 font-medium text-black whitespace-nowrap">End Date</th>
                <th className="py-3 px-3 sm:px-4 font-medium text-black whitespace-nowrap">Requested Student</th>
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {allReservations && allReservations.length !== 0 ? allReservations.map(data => {
                let startDate = new Date(data.fromDate).toLocaleDateString()
                let endDate = new Date(data.toDate).toLocaleDateString()
                return(
                <tr key={data._id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="py-4 px-3 sm:px-4 text-gray-900 whitespace-nowrap">{data.equipment.category}</td>
                  <td className="py-4 px-3 sm:px-4 text-gray-900 whitespace-nowrap">{startDate}</td>
                  <td className="py-4 px-3 sm:px-4 text-gray-900 whitespace-nowrap">{endDate}</td>
                  <td className="py-4 px-3 sm:px-4 text-gray-900 whitespace-nowrap">{data.user.name}</td>
                    ...
                    {data.status === 'PENDING' ? (
                      <div className="flex items-center space-x-2 sm:space-x-3">
                        <button 
                          ...
                          onClick={() => acceptEquipmentHandler(data._id)}
                          aria-label="Accept reservation"
                        >
                          <Tooltip text="Accept">
                            <AcceptIcon className="h-5 w-5 text-[#3c6e71]" />
                          </Tooltip>
                        </button>
                        <button 
                          className="p-2 rounded-md hover:bg-gray-100 transition-colors" 
                          onClick={() => rejectEquipmentHandler(data._id)}
                          aria-label="Reject reservation"
                        >
                          <Tooltip text="Reject">
                            <RejectIcon className="h-5 w-5 text-[#9e2a2b]" />
                          </Tooltip>
                        </button>
                      </div>
                    ) : (
                  ...
                  <td colSpan="7" className="text-center py-12 px-4">
                    <div className='flex flex-col items-center justify-center'>
                      ...
                      <p className='text-xs sm:text-sm text-gray-500 px-4'>
                        {hasActiveFilters ? 'Try adjusting your search or filters' : 'There are no reservations yet'}
                      </p>
                      {hasActiveFilters && (
                        <button
                          onClick={handleClearFilters}
