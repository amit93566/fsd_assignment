...
function StudentList() {
    ...
    useEffect(() => {
        getAllStudentsHandler(debouncedSearchQuery, currentPage);
    }, [debouncedSearchQuery, currentPage]);

    function getAllStudentsHandler(searchQuery, page = 1) {
        setLoading(true);
    ...
    return (
        <div className='py-4 px-4 sm:px-6 lg:px-10 max-w-7xl mx-auto'>
            <h1 className='text-2xl sm:text-3xl font-semibold mb-6'>Student List</h1>
                            ...
                            {students && students.length !== 0 ? students.map(data => {
                                ...
                                return (
                                    <tr key={data._id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="py-4 px-3 sm:px-4 text-gray-900 whitespace-nowrap">{createdAt}</td>
                                    </tr>
                                )
                            }) : (
                                <tr>
                                    <td colSpan="4" className="text-center py-12 px-4">
                                        <div className='flex flex-col items-center justify-center'>
                                            ...
                                            <p className='text-xs sm:text-sm text-gray-500 px-4'>
                                                {searchQuery ? 'Try a different email address' : 'There are no students registered yet'}
                                            </p>
                                            {searchQuery && (
                                                <button
                                                    onClick={() => setSearchQuery('')}
